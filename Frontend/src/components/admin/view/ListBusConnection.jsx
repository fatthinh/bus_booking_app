...
function ListBusConnection() {
  ...
  const DeleteRow = async (id) => {
    AdminBusAxios.delete(`/bus/connect/delete/${id}`)
      .then((res) => {
        ListConnection();
  ...
  const ListConnection = async () => {
    await AdminBusAxios.get("/bus/connect/list")
      .then((res) => {
        const newArray = res.data.map((item) =>
          createData(
            item.id,
            item.bus_stop_1.stop_name,
            item.distance,
            item.time,
            item.bus_stop_2.stop_name
          )
        );
        setRows(newArray);
      })
  ...
  return (
    <TableContainer component={Paper}>
      <Table sx={{ minWidth: 650 }} aria-label="simple table">
        <TableHead>
          <TableRow>
            <TableCell align="left">Xuất phát</TableCell>
            <TableCell align="left">Khoảng cách</TableCell>
              ...
              >
                ...
                <TableCell align="center">
                  <button
                    ...
                    className="bg-white hover:bg-red-700 text-red-400 font-bold py-1 px-3 rounded-2xl"
                  >
                    Xóa
                  </button>
                </TableCell>
