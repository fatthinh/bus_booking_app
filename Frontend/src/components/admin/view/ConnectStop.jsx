...
function ConnectStop() {
  ...
  useEffect(() => {
    AdminBusAxios.get("/bus/stop/list")
      .then((res) => {
  ...
  const submitBusConnection = (event) => {
    ...
    AdminBusAxios.post("/bus/connect", formData)
      .then((response) => {
  ...
  return (
    <>
      <h1 className="text-center font-bold text-2xl m-6">Chọn tuyến</h1>
      <div className="absolute w-full xl:w-3/4 z-50 top-48">
        <div className="justify-evenly flex">
          <div>
            <select
              className=" text-center sm:w-56 w-32 h-11 focus:outline-none appearance-none rounded-xl shadow-xl"
              onChange={(e) => {
                setStop1(stopNames.find((val) => e.target.value == val.id));
              }}
              name="place1"
            >
              <option className="text-center outline-none" value="">
                Chọn
              </option>
              {stopNames
                .filter((value) => value != stop2)
                .map((val, index) => (
                  <option
          ...
          <div>
            <select
              className="sm:w-56 text-center w-32 h-11 appearance-none focus:outline-none rounded-xl shadow-xl"
              onChange={(e) => {
                setStop2(stopNames.find((val) => e.target.value == val.id));
              }}
              name="place2"
            >
              <option className="text-center outline-none" value="">
      ...
      <form onSubmit={submitBusConnection}>
        ...
        <button
          className="ml-8 mt-10 w-1/12 flex-shrink-0 bg-red-500 hover:bg-red-700 border-red-600 hover:border-red-900 text-sm border-4 text-white py-1 px-2 rounded"
          type="submit"
        >
          Thêm
        </button>
