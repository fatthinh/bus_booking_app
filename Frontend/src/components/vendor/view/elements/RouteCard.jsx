  ...
  return (
    ...
    >
      <div
        ...
        onClick={() => {
          validateSetChosen(Waypoint);
        }}
      >
        <h5 className="mb-2 text-lg font-bold tracking-tight text-gray-900">
          {Waypoint.length} tuyến
        </h5>
        <h5 className="mb-2 text-sm font-bold tracking-tight text-gray-900">
          {Waypoint.length == 1 && Waypoint[0].stop_name}
          {Waypoint.length == 2 &&
            `${Waypoint[0].stop_name} -- ${Waypoint[1].stop_name}`}
          {Waypoint.length == 3 &&
            `${Waypoint[0].stop_name} -- ${Waypoint[1].stop_name} -- ${Waypoint[2].stop_name}`}
          {/* {Waypoint.length == 0 && "Không có tuyến"} */}
          {Waypoint.length > 3 &&
            Waypoint.length < 6 &&
            `${Waypoint[Math.round(Waypoint.length / 2) + 1].stop_name} -- ${
              Waypoint[Math.round(Waypoint.length / 2)].stop_name
            } -- ${Waypoint[Math.round(Waypoint.length / 2) - 1].stop_name}`}
          {Waypoint.length >= 6 &&
            `${Waypoint[Math.round(Waypoint.length / 2) + 2].stop_name} -- ${
              Waypoint[Math.round(Waypoint.length / 2)].stop_name
            } -- ${Waypoint[Math.round(Waypoint.length / 2) - 2].stop_name}`}
        </h5>

        <div className="flex flex-col md:flex-row mb-2">
          <button className="btn ml-0 mt-2 md:ml-2 md:mt-0">Xem tuyến</button>
        </div>
      </div>
    </Link>
  );
