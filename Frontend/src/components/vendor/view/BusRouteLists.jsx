...
function BusRouteLists() {
  ...
  useEffect(() => {
    VendorAxios.get(`/vendor/view/bus/route/${userId}`)
      .then((res) => {
        ...
        <div className="container mx-auto bg-gray-50 min-h-screen p-8 antialiased">
          <div>
            {busRoutes.map((res) => {
              ...
              return (
                <div
                  className="bg-gray-100 mx-auto border-gray-500 border rounded-sm  text-gray-700 mb-0.5"
                  key={res.id}
                >
                  <div className="flex p-3  border-l-8 border-green-600">
                    <div className="space-y-1 border-r-2 pr-3">
                      <div className="text-sm leading-5 font-semibold">
                        ...
                        <span className="ml-2"> {res.id}</span>
                      </div>
                      <div className="text-sm leading-5 font-semibold">
                        <span className="text-xs leading-4 font-normal text-gray-500 pr">
                          Bắt đầu:
                        </span>
                        <span className="ml-2"> {res.starting_time}</span>
                      </div>
                      <div className="text-sm leading-5 font-semibold">
                        <span className="text-xs leading-4 font-normal text-gray-500 pr">
                          Kết thúc:
                        </span>
                        <span className="ml-2"> {res.ending_time}</span>
                      </div>
                    </div>
                    <div className="flex-1">
                      <div className="ml-3 space-y-1 border-r-2 pr-3">
                        ...
                        <div className="text-sm leading-4 font-normal">
                          ...
                          <span className="ml-2">{res.origin.stop_name}</span>
                        </div>
                        <div className="text-sm leading-4 font-normal">
                          <span className="text-xs leading-4 font-normal text-gray-500">
                            Điếm đến
